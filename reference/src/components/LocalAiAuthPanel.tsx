...
export function LocalAiAuthPanel() {
  ...
  const handleSaveMaxConcurrentTasks = async (): Promise<void> => {
    ...
    setInfo(null);
    try {
      const res = await api.localAiAuth.setMaxConcurrentTasks(n);
      if (!res.ok) {
        const body = (await res.json().catch(() => ({}))) as { error?: string };
        setError(body.error ?? `Save failed (HTTP ${res.status})`);
        return;
  ...
  return (
    <div className="space-y-4" data-testid="local-ai-auth-panel">
      ...
      <div className="space-y-2">
        <label
          htmlFor="local-ai-max-concurrent"
          className="block text-sm font-medium text-foreground"
        >
          Tâches simultanées{' '}
          <span className="text-muted-foreground font-normal">
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs text-muted-foreground">
          Chaque instance est un serveur Local AI distinct. Le load balancing se fait en
